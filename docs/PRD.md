...
> **日期**：2026-06-20  
> **状态**：草案，待评审  
> **配套**：`PRODUCT_DESIGN.md`、`GUI_PLAN.md`、`CCUI_STACK.md`、**`AGENT_MODEL.md`**（领域模型）、**`TRUST_LOOP_PLAN.md`**（可信闭环）、**`PACK_SPEC.md`**（整合包）
...
| 保留为主 | 降级为命令面板 / 独立窗口 |
|----------|---------------------------|
| 对话、审查 | Harness（管 instance / pack）、主页、Compare、编排、设置… |

*具体砍并需 UX 评审；原则是减认知，不是减能力。领域模型与 UI 投影见 **`AGENT_MODEL.md` §7**。*
