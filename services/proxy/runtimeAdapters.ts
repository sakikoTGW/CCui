 ...
 * 运行时适配表（万能打包器的 L1 层）。
 *
 * 抓包层（L2，services/proxy/captureProxy + distill）天生通吃任何运行时——
 * 代理坐在"发给模型的请求体"这个唯一汇合点上。真正不通用的只是 L1：
 * 每个引擎把 skills/rules/MCP 放哪不一样。本表声明各家的文件面，
 * 加一个新引擎 = 加一条数据，不写新逻辑。
 *
 * 路径 token：以 `~/` 开头 → 用户主目录；否则相对项目根。
 * 已核实(verified) 的来自各家官方文档；未核实的标 verified:false，宁缺毋编。
 */
...
export const RUNTIME_ADAPTERS: RuntimeAdapter[] = [
  {
    ...
    label: 'CCui',
    verified: true,
    detect: ['.claude/skills', '.ccui', 'services/daemon/daemon.ts'],
  ...
  },
  {
    ...
    skills: [{ token: '.agents/skills' }, { token: '~/.codex/skills' }],
    rules: [
      { kind: 'file', token: 'AGENTS.md', format: 'agents-md' },
  ...
  },
  {
    ...
    label: 'opencode',
    verified: true,
    detect: ['opencode.json', '~/.config/opencode/opencode.json'],
    skills: [{ token: '~/.claude/skills' }], // opencode 复用 Claude skills
    rules: [
      { kind: 'file', token: 'AGENTS.md', format: 'agents-md' },
  ...
  },
  {
    ...
    rules: [],
    mcp: [],
    note: 'AstrBot：skills=data/skills/<名>/SKILL.md（同 Codex/Claude）；CCui 把 pack 包装成插件 data/plugins/ccui-<pack>/skills/。MCP 在 data/cmd_config.json(WebUI)；瓶口 provider.api_base。见 HARNESS_RESEARCH.md §5。',
  },
  {
    id: 'generic-agents',
    label: '通用（AGENTS.md）',
...
const UNIVERSAL_IGNORE = new Set([
  ...
  'coverage', '.turbo', 'site-packages', '.venv', 'venv', '__pycache__', '.ccui',
])
