...
import { resolveSkillDir } from './packPortable.js'
...
function mainPy(pack: PackDoc, dirName: string, skills: string[], mcp: string[]): string {
  ...
  // Python 源（utf-8）。保持简单、可加载；工具端口用确证可用的 register_llm_tool。
  return `import json
import logging
...
class CcuiPack_${dirName}(Star):
    """CCui 整合包「${packName}」挂载插件。

    - skills：本插件 skills/ 子目录中的 SKILL.md 会被 AstrBot 自动发现（skills 端口）。
...
export async function writeAstrbotPlugin(
  ...
  const dirName = `ccui_${pyIdent(pack.name || 'pack')}`
