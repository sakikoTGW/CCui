...
export async function renderAndRun(root: Root, element: React.ReactNode): Promise<void> {
  const {
    devStartupReady
  } = await import('../utils/devStartupProgress.js');
  root.render(element);
  devStartupReady();
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  // Dev fork (CLAUDE_CODE_DEV): skip ALL setup dialogs — local API-key use (DeepSeek etc.).
  if (isEnvTruthy(process.env.CLAUDE_CODE_DEV)) {
    const devConfig = getGlobalConfig();
    if (!devConfig.theme || !devConfig.hasCompletedOnboarding) {
      completeOnboarding();
      if (!devConfig.theme) {
        saveGlobalConfig(c => ({
          ...c,
      ...
      if (getCustomApiKeyStatus(customApiKeyTruncated) === 'new') {
        saveGlobalConfig(c => ({
          ...
          customApiKeyResponses: {
            ...c.customApiKeyResponses,
            approved: [...(c.customApiKeyResponses?.approved ?? []), customApiKeyTruncated]
          }
        }));
      }
    }
