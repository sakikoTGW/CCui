...
export function isAutoMemoryEnabled(): boolean {
  ...
  // CCui dev mode keeps auto-memory on even under SIMPLE.
  if (isEnvTruthy(process.env.CLAUDE_CODE_SIMPLE)) {
    if (isEnvTruthy(process.env.CLAUDE_CODE_DEV) || isEnvTruthy(process.env.CCUI_MEMORY)) {
      // fall through — CCui enhanced memory enabled
    } else {
      return false
    }
  }
