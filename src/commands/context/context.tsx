...
import { formatRecallSummary } from '../../ccui/memory/recallLog.js';
import { isCcuiMemoryEnabled } from '../../ccui/memory/config.js';
import { isCcuiStackEnabled } from '../../ccui/config.js';
import { loadMemoryGraph } from '../../ccui/memory/memoryGraph.js';
import { applyCcuiHeadroom } from '../../ccui/headroom/compress.js';
import { getTolariaVaultPath } from '../../ccui/vault/tolaria.js';
...
export async function call(onDone: LocalJSXCommandOnDone, context: LocalJSXCommandContext): Promise<React.ReactNode> {
  ...
  const headroom = applyCcuiHeadroom(compactedMessages);
  const apiMessages = headroom.messages;
  ...
  const data = await analyzeContextUsage(apiMessages, mainLoopModel, async () => appState.toolPermissionContext, tools, appState.agentDefinitions, terminalWidth, context,
  ...
  let ccuiBlock = '';
  if (isCcuiStackEnabled() || isCcuiMemoryEnabled()) {
    const graph = await loadMemoryGraph();
    const recall = formatRecallSummary();
    ccuiBlock = [
      '',
      '── CCui Stack ──',
      `Memory graph nodes: ${graph.nodes.length}`,
      `Tolaria vault: ${getTolariaVaultPath() ?? '(unset — CCUI_TOLARIA_VAULT)'}`,
      `Headroom saved: ${headroom.bytesSaved} bytes (this view)`,
      recall ?? '(no recall this session)',
    ].join('\n');
  }
  onDone(output + ccuiBlock);
  return null;
}
