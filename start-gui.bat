...
if not exist "%_EL_BIN%" (
  echo [ERROR] Electron not installed. Open a terminal in the apps\desktop folder and run: npm install
  pause
  exit /b 1
)

if not exist "%GUI%\dist\islands.js" (
  echo Building React islands, first run only. Please wait...
  pushd "%GUI%"
  call npm run build:islands
  popd
)
